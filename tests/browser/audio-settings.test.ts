...
`;
  ...
  async function newPage(prefix = '', suffix = '') {
    ...
    await page.addInitScript(prefix + instrument + suffix);
  ...
  async function settings(page: Page, enabled: boolean, volume: number, persisted = true) {
    ...
    assert.ok(
      Math.abs((await page.evaluate<number>('FormulaClock.state.soundVolume')) - volume) < 1e-9,
    );
    assert.ok(Math.abs(Number(await page.locator('#volume').inputValue()) - volume * 100) < 1e-9);
    if (persisted) {
      assert.deepEqual(
        await page.evaluate(script('(key)=>JSON.parse(localStorage.getItem(key))'), key),
        {
          enabled: enabled,
          volume: volume,
        },
  ...
  async function volume(page: Page, percent: number) {
    ...
    await page
      .locator('#volume')
      .evaluate(
        script('(el,v)=>{el.value=v;el.dispatchEvent(new Event("input",{bubbles:true}));}'),
        String(percent),
      );
  ...
  assert.deepEqual(
    (await page.evaluate<{ frequency: number; when: number }[]>('audioVoices')).length,
    1,
  );
  assert.ok(Math.abs((await page.evaluate<number>('audioGains[0].gain.value')) - 0.128) < 1e-7);
  report['checks'].push(
  ...
  assert.deepEqual(await page.evaluate<number>('audioVoices.length'), 0);
  let resumes = await page.evaluate<number>('audioResumeCalls');
  await page.evaluate(
    'document.dispatchEvent(new KeyboardEvent("keydown",{key:"a",bubbles:true}))',
  );
  assert.deepEqual(await page.evaluate<number>('audioResumeCalls'), resumes);
  await page.evaluate('window.holdAudioResume=false');
  await page.click('#settings-open');
  await page.waitForFunction('FormulaClock.state.soundReady', undefined);
  assert.ok((await page.evaluate<number>('audioResumeCalls')) >= 2);
  assert.deepEqual(await page.evaluate<number>('audioVoices.length'), 0);
  assert.ok(Math.abs((await page.evaluate<number>('audioGains[0].gain.value')) - 0.128) < 1e-7);
  await page.keyboard.press('Escape');
  report['checks'].push(
  ...
  await ready(page);
  await settings(page, true, 0);
  resumes = await page.evaluate<number>('audioResumeCalls');
  await page.click('#sound');
  ...
  let baseline = await page.evaluate<number>('audioVoices.length');
  await page.evaluate(
    script('async()=>{await audioContext.suspend();window.holdAudioResume=true;}'),
  );
  await page.click('#sound');
  await settings(page, true, 0);
  resumes = await page.evaluate<number>('audioResumeCalls');
  ...
  await page.waitForFunction('FormulaClock.state.soundReady', undefined);
  await page.waitForTimeout(180);
  assert.deepEqual(await page.evaluate<number>('audioVoices.length'), baseline + 1);
  report['checks'].push(
  ...
  for (const [raw, enabled, level] of cases) {
    page = await newPage(`localStorage.setItem(${JSON.stringify(key)},${JSON.stringify(raw)});`);
    await settings(page, enabled, level, false);
    await page.close();
  }
  page = await newPage(
    'Storage.prototype.getItem=()=>{throw new Error("Storage unavailable")};Storage.prototype.setItem=()=>{throw new Error("Storage unavailable")};',
  );
  await settings(page, false, 0.25, false);
  ...
  await page.addInitScript(
    `localStorage.setItem(${JSON.stringify(key)},'${JSON.stringify({ enabled: true, volume: 0.4 })}');` +
      instrument +
      rejection,
  );
  await page.goto(args.url);
  ...
  page = await newPage(
    `localStorage.setItem(${JSON.stringify(key)},'${JSON.stringify({ enabled: true, volume: 0.35 })}');`,
  ...
  baseline = await page.evaluate<number>('audioVoices.length');
  if (!report['nativeReadyBeforeInteraction']) {
    await page.click('#settings-open');
    await page.waitForFunction('FormulaClock.state.soundReady', undefined);
  }
  assert.ok(Math.abs((await page.evaluate<number>('audioGains[0].gain.value')) - 0.112) < 1e-7);
  assert.deepEqual(await page.evaluate<number>('audioVoices.length'), baseline);
  await page.close();
  report['checks'].push(
