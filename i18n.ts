...
const messages = {
  ...
  shareFailed: {ja:'共有リンクを保存できませんでした。もう一度お試しください。',en:'Could not save the share link. Please try again.'},
  ...
  shareNotFound: {ja:'リンクが見つかりません。作成直後の場合は、少し待ってから再読み込みしてください。',en:'This link was not found. If it was just created, wait a moment and reload.'},
  shareLoadFailed: {ja:'共有データを読み込めませんでした。少し待ってから再読み込みしてください。',en:'Could not load the saved view. Wait a moment and reload.'},
...
function create(language: unknown) {
  ...
  function apply(document: Document) {
    ...
    for (const element of document.querySelectorAll<HTMLElement>('[data-i18n]')) element.textContent = t(element.dataset.i18n as MessageKey);
