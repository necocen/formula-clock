...
test('external build exactly partitions the canonical day and publishes only site assets',()=>{
  ...
  const combined={};
  for(let h=0;h<24;h++){
    const hour=String(h).padStart(2,'0'),url=manifest.hours[hour],raw=read('data/'+url),table=JSON.parse(raw);
  ...
  assert.deepEqual(fs.readdirSync(dir).sort(),['_headers','data','index.html','licenses.html','og-default.png']);
