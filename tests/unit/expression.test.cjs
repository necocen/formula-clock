...
const E = require('../../src/shared/expression.ts'),
  D = require('../../src/shared/data.ts');
const { createSolver } = require('../../tools/solver.ts');
const table = require('../../data/expressions.json'),
  solver = createSolver();
const verifyExact = require('../helpers/verify-exact.cjs'),
  extended = [];
...
function tree(i, j) {
  let x;
  if (j - i === 1 || random() < 0.18) x = { op: 'lit', i, j };
  else {
    const k = i + 1 + Math.floor(random() * (j - i - 1));
    x = { op: choose(['add', 'sub', 'mul', 'div', 'pow']), a: tree(i, k), b: tree(k, j) };
  }
  for (let k = 0; k < 2 && random() < 0.38; k++) x = { op: choose(['sqrt', 'neg', 'fact']), a: x };
  return x;
}
const fuzz = 5000;
...
require('../helpers/report.cjs')('expression-results.json', report);
console.log(report);
