...
Cloudflareへのプレビュー・公開・R2設定は[共有機能の運用](docs/SHARING.md)を参照してください。
...
共有ボタンを押すと表示中の秒で停止し、同じ時刻・フォント・表示形式を復元するURLを共有できます。
受け取った人の保存済み設定は書き換えません。共有URLの形式は[FORMAT.md](FORMAT.md#9-共有urlとog画像)に記載しています。
...
親項目をオフにしても選択値は保持します。初期設定はいずれもオフです。
