...
const validCode = (code: unknown): code is string =>
  typeof code === 'string' && /^(?:[01]\d|2[0-3])[0-5]\d$/.test(code);
function assertCode(code: unknown): asserts code is string {
  if (!validCode(code)) throw new TypeError('HHMM must be a 24-hour time (0000–2359)');
}
  ...
  function visit(x: unknown, depth: number): Expr {
    ...
    if (x.op === 'lit') {
      ...
      if (
        ...
        throw new TypeError('Invalid digit interval');
      if (j - i > 1 && code[i] === '0')
        throw new TypeError('Leading zero in a concatenated number');
      for (let k = i; k < j; k++) slots.push(k);
      out = { op: 'lit', i: i as 0 | 1 | 2 | 3, j: j as 1 | 2 | 3 | 4 };
    } else if (UNARY.has(x.op))
      out = { op: x.op as 'neg' | 'sqrt' | 'fact', a: visit(x.a, depth + 1) };
    else if (BINARY.has(x.op))
      out = {
        op: x.op as 'add' | 'sub' | 'mul' | 'div' | 'pow',
        a: visit(x.a, depth + 1),
        b: visit(x.b, depth + 1),
      };
    else throw new TypeError(`Unknown operation: ${String(x.op)}`);
    const allowed = Object.keys(out);
    if (Object.keys(x).some((key) => !allowed.includes(key)))
      throw new TypeError(`Unexpected expression field (${x.op})`);
    active.delete(x);
    return Object.freeze(out);
  }
  const result = visit(ast, 0);
  if (slots.join(',') !== '0,1,2,3')
    throw new TypeError('Use each HHMM position exactly once, in order');
  return result;
}
...
function options(value: Partial<TexOptions> = {}): TexOptions {
  ...
  return {
    ...
    centerOperators: value.centerOperators !== false,
    symbolMotion: value.symbolMotion === true,
    structureMotion: value.structureMotion === true,
    symbolMorph: value.symbolMorph === true,
  };
}
...
const center = (symbol: string, opt: Pick<TexOptions, 'centerOperators'>) =>
  opt.centerOperators ? `\\vcenter{${symbol}}` : symbol;
const binary = (symbol: string, opt: Pick<TexOptions, 'centerOperators'>) =>
  opt.centerOperators ? `\\mathbin{${center(symbol, opt)}}` : symbol;
const negative = (opt: Pick<TexOptions, 'centerOperators'>) =>
  opt.centerOperators ? '\\mathord{\\vcenter{-}}' : '-';
const relation = (opt: Pick<TexOptions, 'centerOperators'>) =>
  `\\mathrel{\\cssId{fc-eq}{${center('=', opt)}}}`;
...
function precedence(ast: Expr, opt: TexOptions): number {
  if (ast.op === 'lit' || ast.op === 'sqrt' || (ast.op === 'div' && opt.division === 'fraction'))
    return 60;
...
function expressionTex(ast: Expr, code: string, settings: Partial<TexOptions> = {}): string {
  ...
  function write(a: Expr): string {
    if (a.op === 'lit') {
      let out = '';
      for (let i = a.i; i < a.j; i++) out += mark(`d${i}`, code[i], opt);
      return out;
    }
    ...
    if (a.op === 'div' && opt.division === 'fraction')
      return structure('frac', a, `\\frac{${write(a.a)}}{${write(a.b)}}`);
    const p = precedence(a, opt);
    function child(x: Expr, right: boolean): string {
      const q = precedence(x, opt),
        tex = write(x);
...
function frameTex(
  ...
  if (!Number.isInteger(seconds) || seconds < 0 || seconds > 59)
    throw new TypeError('Invalid second');
  const opt = options(settings),
...
function expressionText(
  ast: Expr,
  code: string,
  style: { space: string; minus: string; multiply: string; division: string },
): string {
  const opt = options({ division: 'inline' }),
    group = (text: string) => `(${text})`;
  function write(a: Expr): string {
    ...
    if (a.op === 'pow')
      return (
        (precedence(a.a, opt) <= 40 || a.a.op === 'sqrt' ? group(left) : left) +
        '^' +
    ...
    function child(node: Expr, text: string, isRight: boolean) {
      ...
      const homogeneous = (n: Expr): boolean =>
        ...
        (n.op === a.op && 'b' in n && homogeneous(n.a) && homogeneous(n.b));
      const associative =
        (a.op === 'add' || a.op === 'mul') && node.op === a.op && homogeneous(node);
      return q < p || (isRight && ((q === p && !associative) || node.op === 'neg'))
        ? group(text)
        : text;
    }
...
function plain(ast: Expr, code: string, options: { division?: '÷' | '/' } = {}): string {
  return expressionText(ast, code, {
    space: ' ',
    minus: '−',
    multiply: '×',
