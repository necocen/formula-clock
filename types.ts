...
export type Expr =
  ...
  | { [Op in 'neg' | 'sqrt' | 'fact']: { readonly op: Op; readonly a: Expr } }['neg' | 'sqrt' | 'fact']
  | { [Op in 'add' | 'sub' | 'mul' | 'div' | 'pow']: { readonly op: Op; readonly a: Expr; readonly b: Expr } }['add' | 'sub' | 'mul' | 'div' | 'pow'];
...
export interface TexOptions {
  division: DisplayOptions['division']; oldstyle: boolean; centerOperators: boolean;
  symbolMotion: boolean; structureMotion: boolean; symbolMorph: boolean;
}
export interface SymbolIdentity {
  kind: string; site?: string; glyphKey?: string; font?: string; exiting?: boolean;
}
export interface Typography {
  profile: DisplayOptions['font']; numerals: DisplayOptions['numerals']; axisMode: 'numeric' | 'font';
  referenceDigit: string; originalAxisEm: number; numericAxisEm: number; axisEm: number;
  zeroTop: number; zeroBottom: number; equalCenterY: number;
}
...
export interface LayoutItem {
  slot: string; text: string; matrix: number[]; localMatrix: number[]; scale: number;
}
export interface ClockLayout {
  display: DisplayOptions; ast: Expr | null; code: string; seconds: number; mode: 'formula' | 'time'; tex: string;
  items: LayoutItem[]; width: number; height: number; viewBox: Bounds; fontSize: number; axisY: number;
  localAxisY: number; fit: number[]; typography: Typography;
}
export interface AudioEvent {
  type: 'minute' | 'ten-second' | 'countdown' | 'second'; time: number; frequency: number; duration: number;
}
export interface ClockState {
  display: DisplayOptions; dataRevision: number; dataError: string | null; now: string;
  preview: boolean; paused: boolean; layout: ClockLayout | null; coverage: number | undefined;
  audio: AudioEvent[]; soundEnabled: boolean; soundReady: boolean; soundVolume: number;
  engineReady: boolean; engineError: string | null; typesetCacheSize: number;
}
export interface GlyphDiagnostic {
  text: string | undefined; transform: string | null; x: number; y: number; width: number; height: number;
  inStage: boolean; opacity: string; visibility: string;
}
export interface ClockDiagnostics {
  build: string; mathjax: string | null; display: DisplayOptions; userAgent: string; locale: 'ja' | 'en';
  engineError: string | null; typography: Typography | null; axisY: number | undefined; localAxisY: number | undefined;
  time: string | null; glyphs: GlyphDiagnostic[];
}
