...
- アプリ・Worker・ビルドツール・テストはTypeScriptのES Modules。単体・ビルド・OG検証はVitest、ブラウザ検証はPlaywright Testで実行する。共通の型は `src/shared/types.ts`、ブラウザ固有の型は `src/browser/types.ts` / `src/browser/globals.d.ts`。`strict`を保ち、信頼境界を型とAPIの契約に明記する。管理下の式データの構造は生成・取り込み・ビルド時に検証し、実行時のAST検証・複製・凍結は行わない。公開の共有作成APIが受け取る外部JSONはサーバーで検証する。
...
- パッケージ管理はpnpm 12.4.1。`pnpm install`でNode.js 22.23.2と依存パッケージを準備する。依存は`pnpm-lock.yaml`で管理し、`node_modules/`と`.pnpm/`は生成物としてGitへ入れない。npmなどで別の依存環境・ロックファイルを作らない。CIでは`pnpm install --frozen-lockfile`を使う。
...
- `pnpm run generate` は全日データの再探索。起動・表示変更だけなら実行しない。外部の検証済みデータは`pnpm run import:data DIRECTORY`で24時間分を取り込み、`data/README.md`の出典も更新する。式の計算結果と定義域はデータ生成側で検証し、このリポジトリのテストでは再計算しない。ASTの構造とTeX・テキスト変換の結合順序・数字のスロットは検証する。
...
- 分数/÷/スラッシュの切り替えで計算の意味・数字の順序・時刻を変えない（tests/unit/expression.test.tsのroundtrip検証・clock.test.ts）。
