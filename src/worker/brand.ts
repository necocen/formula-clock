...
function wordmark() {
  const font = new MathJaxFiraFont(),
    paths = [];
  let x = 71;
...
const word = wordmark();
export const BRAND = `<g id="brand" fill="${COLORS.foreground}">${COLORS.digits
  .map(
    (color, i) =>
      `<circle cx="${51.5 + (i % 2) * 7}" cy="${44.5 + Math.floor(i / 2) * 7}" r="1.5" fill="${color}"/>`,
  )
  .join('')}${word.paths}</g>`;
