...
import {initRenderer,renderDefaultOg} from '../worker/render.mjs';

const root = fileURLToPath(new URL('../',import.meta.url));
const output = path.join(root,'dist-worker');
const sources = ['display.js','share.js','expression.js','data.js','data/expressions.json','package-lock.json',
  'worker/index.mjs','worker/handler.mjs','worker/render.mjs','worker/brand.mjs','tools/build-worker.mjs'];
...
await build({absWorkingDir:root,entryPoints:['worker/index.mjs'],outfile:path.join(output,'index.mjs'),bundle:true,minify:true,
  format:'esm',platform:'browser',target:'es2022',define:{OG_RENDER_REVISION:JSON.stringify(revision)},
  plugins:[{name:'compiled-wasm',setup(builder) {
