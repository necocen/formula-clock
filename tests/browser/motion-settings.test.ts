...
declare global {
  ...
  var originalFrame: typeof FormulaTypesetter.Typesetter.prototype.frame;
}
...
test('motion-settings', async ({ browser, args, page }) => {
  ...
  async function check(bits: readonly boolean[], saved = false) {
    ...
    assert.deepEqual(
      await page.evaluate(
        (keys) => keys.map((k) => FormulaClock.state.display[k as keyof DisplayOptions]),
        keys,
      ),
      Array.from(bits),
    );
    ...
    if (saved) {
      assert.deepEqual(
        await page.evaluate((keys) => {
          const s = JSON.parse(localStorage.getItem('formula-clock-display-v2')!);
  ...
  async function preview(time: string) {
    await page.evaluate(
      (time) => FormulaClock.preview('2026-09-09T' + time + '+09:00', true),
      time,
    );
    await page.waitForFunction(
      ([c, s]) =>
      ...
      [time.slice(0, 2) + time.slice(3, 5), Number(time.slice(6))] as [string, number],
    );
  }
  ...
  for (const [basic, structure, morph] of combinations([false, true], 3)) {
    ...
    await page.evaluate(
      (raw) => localStorage.setItem('formula-clock-display-v2', JSON.stringify(raw)),
      raw,
    );
    await page.reload();
    await ready();
  ...
  const before = await page.evaluate(() => FormulaClock.state.display);
  assert.ok(
    await page.evaluate(async () => {
      try {
        await FormulaClock.setDisplay({
          symbolMotion: false,
          structureMotion: 'invalid' as unknown as boolean,
          symbolMorph: true,
        });
        return false;
      } catch (e) {
  ...
  assert.deepEqual(await page.evaluate(() => FormulaClock.state.display), before);
  await page.evaluate(() => {
  ...
  for (const [font, numerals] of [
    ...
    for (const time of ['12:34:08', '12:34:59', '00:00:08']) {
      ...
      await page.waitForFunction(
        ...
        [font, numerals] as [DisplayOptions['font'], DisplayOptions['numerals']],
      );
      assert.ok(!(await page.evaluate(() => FormulaClock.state.engineError)));
      assert.ok(
        (await page.evaluate(() => FormulaClock.diagnostics().glyphs))
          .map((g) => g['inStage'])
          .every(Boolean),
      );
      assert.notDeepEqual(await page.locator('#render-status').getAttribute('hidden'), null);
  ...
  await page.evaluate(() => {
    const L = (i: number): Expr => ({
        op: 'lit',
        i: i as 0 | 1 | 2 | 3,
        j: (i + 1) as 1 | 2 | 3 | 4,
      }),
      B = (op: 'add' | 'mul', a: Expr, b: Expr): Expr => ({ op, a, b });
    ...
    FormulaClock.setDataProvider({
      async getMinute(hhmm) {
        return {
          ...
          hhmm,
          seconds: Array.from({ length: 60 }, (_, s) =>
            hhmm === '1234' ? formulas[s] || null : null,
          ),
        };
      },
    });
  ...
  assert.ok(
    Number(
      await page
        ...
        .evaluate((el) => getComputedStyle(el.closest('label')!).opacity),
    ) < 0.6,
  );
  ...
  await page.evaluate(() => {
    const prototype = FormulaTypesetter.Typesetter.prototype;
    window.originalFrame = prototype.frame;
    prototype.frame = function (
      this: InstanceType<typeof FormulaTypesetter.Typesetter>,
      ast,
      ...args
    ) {
      return ast
        ? Promise.reject(new Error('Injected expression failure'))
        : originalFrame.call(this, ast, ...args);
    };
    FormulaClock.setDisplay({});
  });
  await page.waitForFunction(
    () =>
      FormulaClock.state.engineError?.includes('Injected expression failure') &&
      FormulaClock.state.layout!.mode === 'time',
    undefined,
  );
  ...
  await page.evaluate(() => {
    FormulaTypesetter.Typesetter.prototype.frame = originalFrame;
    return FormulaClock.setDisplay({});
  });
  await page.waitForFunction(
