...
(async () => {
  ...
  try {
    global.fetch = async (url, options) => {
      seen = { url, options };
      return { ok: true, json: async () => minute };
    };
    ...
    global.fetch = async () => ({ ok: true, json: async () => ({ ...minute, hhmm: '1235' }) });
  ...
  console.log(report);
  require('../helpers/report.cjs')('provider-results.json', report);
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
