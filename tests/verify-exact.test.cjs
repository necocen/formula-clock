...
const verify=require('./verify-exact.cjs');
const {validateAst}=require('../expression.ts');
const L=(i,j=i+1)=>({op:'lit',i,j}),U=(op,a)=>({op,a}),B=(op,a,b)=>({op,a,b});
test('symbolic verification accepts exact extended expressions and rejects invalid intermediate domains and near misses',()=>{
  ...
  const cases=[
    ['0020',32,B('pow',U('sqrt',U('sqrt',B('add',U('fact',L(0)),U('fact',L(1))))),L(2,4)),true],
    ...
    ['0020',31,B('pow',U('sqrt',U('sqrt',B('add',U('fact',L(0)),U('fact',L(1))))),L(2,4)),false]
  ];
  const result=verify(cases.map(([code,second,ast])=>({code,second,ast:validateAst(ast,code)})));
  assert.deepEqual(result.results.map(row=>row.valid),cases.map(row=>row[3]));
});
