...
test('fullscreen', async ({ browser, args }) => {
  ...
  for (const scenario of ['native', 'unavailable', 'policy-disabled', 'rejected', 'prefixed']) {
    ...
    if (['unavailable', 'policy-disabled'].includes(scenario)) {
      await ctx.addInitScript(`Object.defineProperty(document,'fullscreenEnabled',{value:false});
              Object.defineProperty(document,'webkitFullscreenEnabled',{value:false});`);
    }
    if (scenario === 'unavailable') {
      await ctx.addInitScript(`Element.prototype.requestFullscreen=undefined;
              Element.prototype.webkitRequestFullscreen=undefined;`);
    }
    if (scenario === 'rejected') {
      await ctx.addInitScript(`Object.defineProperty(document,'fullscreenEnabled',{value:true});
    ...
    if (['unavailable', 'policy-disabled'].includes(scenario)) {
      ...
      if (scenario === 'rejected') {
        ...
        await page.waitForFunction(
          'document.querySelector("#share-status").textContent.includes("切り替えられませんでした")',
          undefined,
        );
    ...
    if (scenario === 'native') {
      await page.waitForFunction(
        'FormulaClock.state.layout && !document.querySelector("#share").disabled',
        undefined,
        { timeout: 45000 },
      );
      ...
      for (const [t, font, mode] of [
        ...
        await page.evaluate(
          script(
            '([t,font])=>{FormulaClock.setDisplay({font});FormulaClock.preview(FormulaShare.localDate(t),true)}',
          ),
          [t, font],
        );
        await page.waitForFunction(
          script(
            '([t,font])=>{const l=FormulaClock.state.layout;return l?.code===t.slice(0,4)&&l.seconds===Number(t.slice(4))&&l.display.font===font&&!document.querySelector("#share").disabled}',
          ),
          [t, font],
          { timeout: 45000 },
        );
