...
test('transport', async ({ browser, args }) => {
  ...
  async function ready(page: Page, time: string) {
    await page.waitForFunction(
      (t) => {
        ...
        return (
          !!l &&
          l.code + String(l.seconds).padStart(2, '0') === t &&
  ...
  async function preview(page: Page, time: string) {
    await page.evaluate((t) => FormulaClock.preview(FormulaShare.localDate(t)), time);
  ...
  for (const locale of ['ja-JP', 'en-US']) {
    ...
    await ctx.addInitScript(() => {
      ...
      window.Date = class extends NativeDate {
        constructor(...args: unknown[]) {
          super(...((args.length ? args : [window.testWall]) as [number]));
        }
        static now() {
          return window.testWall;
        }
      } as DateConstructor;
    });
    ...
    let frozen = await page.evaluate(() => FormulaClock.state.now);
    await page.waitForTimeout(1100);
    assert.deepEqual(await page.evaluate(() => FormulaClock.state.now), frozen);
    ...
    frozen = await page.evaluate(() => FormulaClock.state.now);
    ...
    await page.evaluate(() => {
      ...
      document.querySelector<HTMLInputElement>('#share-url')!.focus();
    });
    await page.keyboard.press('ArrowRight');
    ...
    for (const [time, font, mode] of [
      ...
      await page.evaluate((f) => FormulaClock.setDisplay({ font: f, division: 'fraction' }), font);
      await preview(page, time);
      await page.waitForFunction((f) => FormulaClock.state.layout!.display.font === f, font, {
        timeout: 45000,
      });
      ...
      assert.ok(
        await page.evaluate(
          () =>
            ...
            FormulaClock.diagnostics().glyphs.every((g) => g.inStage),
        ),
      );
    }
    ...
    await page.waitForTimeout(750);
    assert.ok(
      await page.locator('#transport').evaluate((el) => {
