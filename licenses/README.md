...
pnpm run build              # 単体HTMLへ組み込み
pnpm run build:external     # 配信用HTMLへ組み込み
```
...
4. `pnpm run generate:licenses`と`pnpm test`を実行し、ブラウザのLICENSE画面を確認します。CDNや書体も更新した場合は、`tests/README.md`の該当ブラウザテストも実行します。
