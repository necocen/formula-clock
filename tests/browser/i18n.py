...
from _support import ROOT, browser_args
...
legal = [html.unescape(text).removeprefix('\n') for text in re.findall(r'<pre(?: [^>]*)?>(.*?)</pre>', (ROOT/'src/browser/app.html').read_text(), re.S)]
...
with sync_playwright() as p:
    ...
    page = ctx.new_page()
    page.goto((ROOT/'dist/standalone/index.html').as_uri()+query)
    ...
    assert page.url == (ROOT/'dist/standalone/index.html').as_uri()
