...
interface Value {n: number; d: number; r: number; key: string; v: number;}
interface SolverState extends Value {cost: number; ast: Expr; depth: number;}
interface Found {ast: Expr; cost: number;}
...
export function createSolver() {
  ...
  function inverse(a: Value) { return a.n ? norm(a.d, a.n * a.r, a.r) : null; }
  function div(a: Value, b: Value) { const inv = inverse(b); return inv && mul(a, inv); }
  function power(a: Value, b: Value) {
    ...
    if (!base) return null;
    while (k) {
      if (k % 2) { const product = mul(res, base); if (!product) return null; res = product; }
      k = Math.floor(k / 2);
  ...
  function squareParts(x: number): [number,number] | null {
    ...
    inside *= u;
    const answer: [number,number] = [outside, inside]; squareCache.set(x, answer); return answer;
  }
  ...
  const unaryOps: [UnaryOp,number][] = [['neg', 0.8], ['sqrt', 1.55], ['fact', 1.9]];
  const binaryOps: [BinaryOp,number][] = [['add', 1], ['sub', 1.05], ['mul', 1.2], ['div', 1.8], ['pow', 2.3]];
  ...
  function leaves(a: Expr, out: number[] = []): number[] {
    ...
    else { leaves(a.a, out); if ('b' in a) leaves(a.b, out); }
    return out;
  }
  ...
  function solve(input: string) {
    ...
    for (let len = 1; len <= 4; len++) for (let i = 0; i + len <= 4; i++) {
      const j = i + len, map = new Map<string,SolverState>(), full = len === 4;
      function insert(v: Value | null, cost: number, make: () => Expr, depth = 0, parentOp: UnaryOp | '' = '') {
      ...
      if (len === 1 || digits[i] !== 0) insert(norm(Number(input.slice(i, j))), (len - 1) * 0.28, () => ({ op: 'lit', i:i as 0|1|2|3, j:j as 1|2|3|4 }));
      for (let k = i + 1; k < j; k++) {
      ...
      if (!full && states.length > CAPS[len]) {
        const score = (v: SolverState) => v.cost + (v.r === 1 ? 0 : 1.7) + Math.log2(1 + Math.abs(v.v)) * 0.09 + Math.log2(v.d) * 0.04;
    ...
    if (input === '1234') {
      const L = (i: 0|1|2|3, j = i + 1): Expr => ({ op: 'lit', i:i as 0|1|2|3, j:j as 1|2|3|4 });
