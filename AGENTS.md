...
- 共有ページのタイトルはASTの数式テキストと秒の等式にする。OG／Twitterカードはタイトルを`Formula Clock - HH:MM:SS`、Descriptionを`-(2x3)+50=44`のような空白なしの式にする。ネイティブ共有でも同じメタデータ生成処理を使い、`title`へカードのタイトル、`text`へDescription、`url`へ共有URLを渡す。除算は`/`、累乗は`^`、乗算は`x`、負号と減算は`-`。式がない場合はDescriptionに時刻を使う。
