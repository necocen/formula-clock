import {isRecord, type FormulaProvider, type MinuteRecord, type FetchHourOptions} from './types.ts';
type ReadOptions = {signal?: AbortSignal};
// Only the header is checked here; each minute/AST is validated when consumed.
interface RawTable {schema: 'formula-clock/1'; minutes: Record<string,unknown>;}
interface Manifest {version: string; urls: Readonly<Record<string,string>>;}
...
function checkAbort(signal?: AbortSignal) { if (signal?.aborted) throw new DOMException('Aborted','AbortError'); }
function normalizeMinute(record: unknown, expectedCode: string): MinuteRecord {
  Expr.assertCode(expectedCode);
  if (!isRecord(record) || record.schema !== SCHEMA || record.hhmm !== expectedCode || !Array.isArray(record.seconds) || record.seconds.length !== 60) {
...
class InlineProvider implements FormulaProvider {
  ...
  constructor(table: unknown) { tableHeader(table); this.table = table; }
...
class DataHTTPError extends Error {
  ...
  constructor(status: number, url: string) { super(`Formula data HTTP ${status} (${url})`); this.status=status; }
}
...
class FetchHourProvider implements FormulaProvider {
  ...
  constructor(manifestUrl: string | URL, {fetch: fetcher = (...args) => fetch(...args)}: FetchHourOptions = {}) {
    if (typeof fetcher !== 'function') throw new TypeError('Expected a fetch function');
    this.fetch = fetcher;
    this.url = new URL(manifestUrl,typeof document === 'undefined' ? undefined : document.baseURI).href;
  }
  private async loadManifest(previous?: Manifest): Promise<Manifest> {
    ...
    const task = (async () => {
      ...
      if (!isRecord(raw) || raw.schema !== 'formula-clock-hours/1' || typeof raw.version !== 'string' || !/^[a-f0-9]{64}$/.test(raw.version) ||
          !isRecord(raw.hours) || Object.keys(raw.hours).length !== 24) {
  ...
  async getMinute(hhmm: string, {signal}: ReadOptions = {}): Promise<MinuteRecord> {
    ...
    for (let attempt=0;attempt<2;attempt++) {
      ...
      try {
        ...
        checkAbort(signal);
        if (attempt || (!(error instanceof StaleManifestError) && !(error instanceof DataHTTPError && error.status === 404))) throw error;
...
export {SCHEMA,normalizeMinute,loadEmbedded,InlineProvider,FetchMinuteProvider,FetchHourProvider,TableProvider};
