...
declare global {
  ...
  var oldPluses: Element[];
  var oldMinus: Element;
  var oldPlus: Element;
  var fixedPlus: Element;
  var oldFactorial: Element;
  var factorialShape: ChildNode | null;
}
...
test('symbol-motion', async ({ browser, args, page }) => {
  ...
  async function preview(time: string, wait = 720) {
    ...
    await page.waitForFunction(
      ([code, seconds]) =>
        FormulaClock.state.layout!.code === code && FormulaClock.state.layout!.seconds === seconds,
    ...
    await page.waitForFunction(
      (opts) =>
        Object.entries(opts).every(
          ([key, value]) =>
            FormulaClock.state.layout!.display[key as keyof DisplayOptions] === value,
        ),
      opts,
    );
  ...
  async function checkSymbols() {
    return await page.evaluate(() => {
      ...
      const kinds: Record<string, string> = {
        ...
        mul: '×',
        fact: '!',
        ...(layout.display.division === 'inline' ? { div: '÷' } : {}),
      };
      function walk(ast: { op: string; a?: Expr; b?: Expr } | null) {
        if (!ast) return;
        if (kinds[ast.op]) expected[kinds[ast.op]] = (expected[kinds[ast.op]] || 0) + 1;
        if (ast.a) walk(ast.a);
        if (ast.b) walk(ast.b);
      }
      if (layout.display.symbolMotion) walk(layout.ast);
      const actual: Record<string, number> = {};
      for (const el of document.querySelectorAll<SVGGElement>('#operator-root > g')) {
        actual[el.dataset.kind!] = (actual[el.dataset.kind!] || 0) + 1;
        const r = el.getBoundingClientRect();
        if (!r.width || !r.height || r.right < 0 || r.left > innerWidth)
          throw new Error('Invisible moving symbol');
        if (getComputedStyle(el.querySelector('path')!).fill !== 'rgb(195, 200, 186)')
          throw new Error('Wrong symbol color');
      }
      if (
        JSON.stringify(Object.entries(expected).sort()) !==
        JSON.stringify(Object.entries(actual).sort())
      )
        throw new Error(JSON.stringify({ expected, actual }));
      if (
        !FormulaClock.digits.every((el, i) => el === originalDigits[i]) ||
        document.querySelector('#equal-sign') !== originalEqual
      )
        throw new Error('Persistent digits/equality replaced');
      return actual;
    });
  }
  ...
  const samples = await page.evaluate(async () => {
    const plus = document.querySelector('#operator-root [data-kind="+"]')!,
      shape = plus.firstChild,
      out: { same: boolean; x: number; opacity: string }[] = [];
    FormulaClock.preview('2026-09-08T12:34:31+09:00');
    const started = performance.now();
    while (performance.now() - started < 850) {
      await new Promise(requestAnimationFrame);
      out.push({
        same: plus.isConnected && plus.firstChild === shape,
        x: plus.getBoundingClientRect().x,
        opacity: getComputedStyle(plus).opacity,
      });
    }
    return out;
  });
  ...
  await page.evaluate(() => {
    const L = (i: number) => ({ op: 'lit', i, j: i + 1 }) as Expr,
      B = (op: 'add' | 'sub' | 'mul', a: Expr, b: Expr) => ({ op, a, b }) as Expr,
      F = (i: number) => ({ op: 'fact', a: L(i) }) as Expr;
      ...
      async getMinute(hhmm) {
        return {
          ...
          seconds: Array.from({ length: 60 }, (_, i) =>
            hhmm === '1234' ? formulas[i] || null : null,
          ),
        };
      },
    });
  ...
  for (const [font, numerals] of (['stix2', 'termes', 'fira', 'euler'] as const).flatMap((f) =>
    ...
    assert.ok(
      await page.evaluate(
        () => !oldMinus.isConnected && !oldPlus.isConnected && fixedPlus.isConnected,
      ),
    );
    ...
    await page.evaluate(() => {
      window.oldFactorial = document.querySelector('[data-site="fact-u23-0"]')!;
      window.factorialShape = oldFactorial.firstChild;
    });
    await preview('12:34:14');
    await checkSymbols();
    assert.ok(
      await page.evaluate(
        () => oldFactorial.isConnected && oldFactorial.firstChild === factorialShape,
      ),
    );
    await preview('12:34:30');
    await checkSymbols();
