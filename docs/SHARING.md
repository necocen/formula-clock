...
pnpm 12.4.1を用意し、`pnpm install`を実行する。Node.js 22.23.2と依存パッケージはpnpmが準備する。CIでは`pnpm install --frozen-lockfile`でロック済みの依存を導入する。
...
ID発行中の通知は表示しない。発行の失敗では画面を停止したまま理由を通知し、再試行を可能にする。コピー完了の通知も維持する。受理後の保存はブラウザの状態変更に関係なく続き、失敗しても共有シートを取り消したりブラウザへ通知したりはしない。

保存の発行は共有ボタンのクリック時に行う。通常の時計更新やホバーではKVへ書かない。
発行済み、または発行中の要求は時刻・フォント・数字・除算・ASTの完全一致で再利用する。クリック時にIDが発行済みなら同じイベント内でネイティブ共有を呼び、発行中なら同じ要求を待つ。発行したが共有されなかったレコードも期限なしで残る。
...
共有シートへは`navigator.share({title,url})`で、表示中のスナップショットから作ったカードのタイトルと共有URLだけを直接渡す。`text`は渡さず、数式は共有先のOG画像・Descriptionで扱う。フロントエンドとWorkerは同じ`FormulaShare.card`を使う。タブの`document.title`や共有URLのHTML取得に依存せず、KV保存・OG画像生成の完了を待たない。Web Share APIにプレビュー専用画像を渡す項目はなく、画像ファイルは添付しない。
