...
test('audio', { timeout: 900000 }, async (t) => {
  ...
  const baseline = await page.evaluate<number>('audioVoices.length');
  for (const second of Array.from({ length: 60 }, (_, i) => i)) {
    await wall(second);
    await page.waitForFunction(script('(n)=>audioVoices.length>=n'), baseline + second + 1);
    await page.waitForTimeout(75);
    assert.deepEqual(
      await page.evaluate<number>('audioVoices.length'),
      baseline + second + 1,
      inspect(second),
    );
  }
  const voices = (await page.evaluate<Voice[]>('audioSnapshot()')).slice(baseline);
  ...
  for (const [second, name] of [
    ...
    const result = await page.evaluate<RenderedVoice, unknown>(
      script('(index)=>renderVoice(index)'),
      baseline + second,
    );
    const wav = (() => {
  ...
  let before = await page.evaluate<number>('audioVoices.length');
  await wall(5, 35, 500);
  await page.waitForTimeout(200);
  assert.deepEqual(await page.evaluate<number>('audioVoices.length'), before);
  await wall(6, 35);
  await page.waitForFunction(script('(n)=>audioVoices.length===n'), before + 1);
  assert.deepEqual(await page.evaluate('audioVoices.at(-1).frequency'), 2000);
  before = await page.evaluate<number>('audioVoices.length');
  await page.evaluate(
    "Object.defineProperty(document,'hidden',{configurable:true,value:true});document.dispatchEvent(new Event('visibilitychange'));delete document.hidden;document.dispatchEvent(new Event('visibilitychange'))",
  );
  await page.waitForTimeout(200);
  ...
  await page.click('#sound');
  const cancelled = await page.evaluate<Voice>('audioSnapshot().at(-1)');
  assert.ok(
  ...
  assert.deepEqual(await page.evaluate<number>('audioVoices.length'), before + 1);
  await page.click('#sound');
  before = await page.evaluate<number>('audioVoices.length');
  await wall(11, 35);
  await page.waitForTimeout(200);
  ...
  await page
    .locator('#volume')
    .evaluate(script("el=>{el.value='40';el.dispatchEvent(new Event('input',{bubbles:true}));}"));
  await page.keyboard.press('Escape');
  await page.waitForTimeout(250);
  const volume = await page.evaluate<{ value: number; commands: AudioCommand[]; input: string }>(
    '({value:audioGains[0].gain.value,commands:audioGains[0].commands,input:document.querySelector("#volume").value})',
  );
  ...
  await page.waitForFunction(script('(n)=>audioVoices.length===n+1'), before);
  assert.deepEqual(await page.evaluate('audioVoices.at(-1).frequency'), 500);
  await wall(30, 35);
  await page.waitForFunction(script('(n)=>audioVoices.length===n+2'), before);
  assert.deepEqual(await page.evaluate('audioVoices.at(-1).frequency'), 1000);
