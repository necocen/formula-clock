...
共有シートへは`navigator.share({title,url})`で表示中のスナップショットから作ったタイトルを直接渡す。タブの`document.title`や共有URLのHTML取得に依存せず、KV保存・OG画像生成の完了を待たない。Web Share APIにプレビュー専用画像を渡す項目はなく、画像ファイルは添付しない。
...
ページとネイティブ共有のタイトルは等式テキスト（`1 + 2^3 / √4 = 5`など）。タイトルとDescriptionは同じ優先順位・結合順序の規則で不要な括弧を省く。旧クエリURLのメタデータだけはASSETSから現在の式データを取得し、失敗時は時刻へ戻す。
