...
引数なしでは設定済みの全ブラウザとOG画像差分を実行します。Chromium・WebKitは全16スイート、Firefoxは対応する13スイートです。テストファイル名か`--grep`で絞り込み、`--project`でブラウザを選択します。`share`など名前が重なる場合は`--grep '^share$'`でテスト名を完全一致させてください。アニメーションの計測に競合が出ないよう、実行workerは1に固定しています。
...
| `viewport`                                            | 縦置き・低い横置き・回転・スクロール時の収まり・ヘッダー・操作欄 |
