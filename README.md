...
表示設定と時報の音量はブラウザに保存します。時報のオン／オフは保存せず、開き直し・リロード時は必ずオフから始まります。
...
時報は117風の秒音・予告音・時報音です。時報ボタンまたはMキーでオン／オフを切り替え、その状態はページを開いている間だけ保持します。ほかのクリックやキー操作では開始しません。
開始中は時報ボタンに進行表示が出て、もう一度押すと取り消せます。再生可能になってからオン表示と短い確認音を出し、開始に失敗した場合はオフに戻って通知します。同じボタンで再試行できます。
