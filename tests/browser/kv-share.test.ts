...
import type { Expr, SharedSnapshot } from '../../src/shared/types.ts';
import { test, createReport, playwrightVersion } from '../helpers/browser.ts';

declare global {
  var postBodies: SharedSnapshot[];
  var shareMode: 'normal' | 'fail' | 'slow';
  var nativeCalls: { url: string; title: string; text?: string; activation: boolean }[];
  var copies: string[];
  var sameDocument: boolean;
  var initialHistoryLength: number;
}
...
test('kv-share', async ({ browser, args, context: ctx }) => {
  ...
  async function ready(page: Page, time: string) {
    await page.waitForFunction(
      (time) => {
        const l = window.FormulaClock?.state.layout;
        return (
          l &&
          l.code + String(l.seconds).padStart(2, '0') === time &&
  ...
  await ctx.addInitScript(() => {
    window.postBodies = [];
    window.shareMode = 'normal';
    window.nativeCalls = [];
    window.copies = [];
    const originalFetch = window.fetch;
    window.fetch = async function (this: unknown, input, init) {
      if (String((input as Request)?.url || input) === '/api/shares') {
    ...
    Object.defineProperty(navigator, 'share', {
      ...
      value: (data: { url: string; title: string; text?: string }) => {
        nativeCalls.push({ ...data, activation: navigator.userActivation.isActive });
  ...
  assert.deepEqual(await page.evaluate(() => FormulaClock.state.layout!.ast), ast);
  ...
  const current = await page.evaluate(
    async () => (await FORMULA_CLOCK_CONFIG.provider!.getMinute('1234')).seconds[21],
  );
  assert.notDeepEqual(
    current,
    ast,
  ...
  assert.deepEqual(await page.evaluate(() => nativeCalls.at(-1)!.url), url);
  assert.deepEqual(
    await page.evaluate(() => nativeCalls.at(-1)!.title),
    'Formula Clock - 12:34:21',
  );
  assert.ok(await page.evaluate(() => !('text' in nativeCalls.at(-1)!)));
  assert.ok(await page.evaluate(() => nativeCalls.at(-1)!.activation));
  assert.deepEqual(await page.evaluate(() => postBodies.length), 0);
  ...
  await page.waitForFunction(
    () =>
      FormulaClock.state.layout!.display.font === 'termes' &&
      !document.querySelector<HTMLButtonElement>('#share')!.disabled,
    undefined,
    { timeout: 45000 },
  );
  ...
  await page.waitForFunction(
    () =>
      nativeCalls.length === 2 || document.querySelector<HTMLDialogElement>('#share-dialog')!.open,
    undefined,
  );
  ...
  const restyled = await page.evaluate(() => nativeCalls.at(-1)!.url);
  ...
  assert.ok(
    await page.evaluate(
      async () =>
        JSON.stringify(FormulaClock.state.layout!.ast) ===
        JSON.stringify((await FORMULA_CLOCK_CONFIG.provider!.getMinute('1234')).seconds[21]),
    ),
  );
  assert.notDeepEqual(await page.evaluate(() => FormulaClock.state.layout!.ast), ast);
  assert.ok(
  ...
  assert.deepEqual(
    await page.evaluate(() => nativeCalls.at(-1)!.title),
    'Formula Clock - 12:34:30',
  );
  assert.ok(await page.evaluate(() => !('text' in nativeCalls.at(-1)!)));
  assert.ok(
    await page.evaluate(
      async () => !!(await FORMULA_CLOCK_CONFIG.provider!.getMinute('1234')).seconds[30],
    ),
  );
  ...
  await failedCtx.addInitScript(() => {
    ...
    window.fetch = (input, ...args) =>
      String((input as Request)?.url || input).includes('data/')
        ? Promise.reject(new Error('Dataset offline'))
  ...
  assert.deepEqual(await failedPage.evaluate(() => FormulaClock.state.layout!.ast), ast);
  assert.deepEqual(
    await failedPage.evaluate(() => new Date(FormulaClock.state.now).getHours()),
    12,
  );
  ...
  await page.evaluate(() => {
    window.nativeCalls = [];
    window.shareMode = 'slow';
    FormulaClock.preview(FormulaShare.localDate('123431'), true);
  });
  ...
  assert.ok(
    await page.evaluate(
      () =>
        FormulaClock.state.paused && document.querySelector<HTMLButtonElement>('#share')!.disabled,
    ),
  );
  ...
  assert.deepEqual(await page.evaluate(() => nativeCalls.length), 0);
  for (const width of [320, 390, 768]) {
    await page.setViewportSize({ width: width, height: 844 });
    assert.ok(
      await page.locator('#share-dialog').evaluate((el) => el.scrollWidth <= el.clientWidth),
    );
  ...
  await page.click('#share-native');
  assert.ok(await page.evaluate(() => nativeCalls.at(-1)!.activation));
  const delayedUrl = await page.evaluate(() => nativeCalls.at(-1)!.url);
  assert.ok(new RegExp('/s/[A-Za-z0-9]{10}$').exec(delayedUrl)!);
  assert.deepEqual(
    await readSnapshot(ctx, delayedUrl),
  ...
  await page.evaluate(() => {
    window.shareMode = 'fail';
    FormulaClock.preview(FormulaShare.localDate('123432'), true);
  });
  await ready(page, '123432');
  await page.click('#share');
  await page.waitForFunction(
    () =>
      document.querySelector('#share-status')!.textContent!.includes('作成できません') &&
      !document.querySelector<HTMLButtonElement>('#share')!.disabled,
    undefined,
  );
  assert.ok(
  ...
  for (const width of [320, 390]) {
    ...
    assert.ok(
      await page
        ...
        .evaluate(
          (el) =>
            el.getBoundingClientRect().left >= 0 && el.getBoundingClientRect().right <= innerWidth,
        ),
    );
  }
  await page.evaluate(() => {
  ...
  assert.deepEqual(
    await readSnapshot(ctx, await page.evaluate(() => copies.at(-1)!)),
    await page.evaluate(() => postBodies.at(-1)),
  );
  await page.evaluate(() => {
    window.shareMode = 'slow';
    FormulaClock.preview(FormulaShare.localDate('123433'), true);
  });
  ...
  assert.deepEqual(await page.evaluate(() => copies.length), 1);
  ...
  assert.ok(await page.evaluate(() => !('text' in nativeCalls.at(-1)!)));
  assert.deepEqual(await page.evaluate(() => nativeCalls.at(-1)!.url), args.url + 's/' + titleId);
  report['checks'].push(
