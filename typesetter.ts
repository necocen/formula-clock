import type {Bounds,DisplayOptions,Expr,TexOptions,Typography} from './types.ts';
import type {ClockFace,Frame,GlyphToken,PlacedToken,MathJaxRuntime} from './browser-types.ts';
...
function finiteMatrix(m: DOMMatrixReadOnly) { return matrixArray(m).every(Number.isFinite); }
...
class Typesetter {
  ...
  constructor(profile: DisplayOptions['font'] = 'stix2', numerals: DisplayOptions['numerals'] = 'oldstyle') {
  ...
  load() {
    return new Promise<void>((resolve, reject) => {
      ...
      const engineWindow = this.host.contentWindow as (Window & typeof globalThis & {MathJax: MathJaxRuntime}) | null;
      const engineDocument = this.host.contentDocument;
      if (!engineWindow || !engineDocument) return finish(new Error('MathJax frame is unavailable'));
      const config = {
        loader: {
          load: ['[tex]/html'],
          failed: (error: unknown) => finish(new Error(`MathJax: ${error instanceof Error ? error.message : String(error)}`))
        },
        tex: { packages: { '[+]': ['html'] } },
      ...
      engineWindow.MathJax.loader.failed = error => finish(new Error(`MathJax: ${error instanceof Error ? error.message : String(error)}`));
  ...
  async calibrateTypography() {
    ...
    try {
      ...
      typography = Object.freeze({
        ...
        numericAxisEm: numericAxis, zeroTop: top, zeroBottom: bottom, axisEm:params.axis_height
      });
  ...
  frame(ast: Expr | null, code: string, seconds: number, settings: Partial<TexOptions> = {}): Promise<Frame> {
  ...
  clockFace(): Promise<ClockFace> {
    if (!this.clockFaceTask) {
      const task = this.tail.then(() => this.boot).then(async () => {
        ...
        for (const [code,seconds] of [['0123',45],['1607',58],['0900',0]] as const) {
  ...
  private async convert(tex: string, code: string, seconds: number, clockFace = false): Promise<Frame> {
    ...
    try {
      ...
      function glyphToken(paths: SVGPathElement[], metadata: Omit<PlacedToken,'matrix'|'shape'>): GlyphToken {
        const {slot} = metadata;
      ...
      const tokens = slots.map((slot, i) => {
        ...
        return glyphToken([...svg.querySelectorAll<SVGPathElement>(`#fc-${slot} path`)], {
          slot, kind:symbol?.kind || (slot === 'eq' ? '=' : 'digit'), text:symbol ? symbol.kind : slot === 'eq' ? '=' : i < 4 ? code[i] : secondsText[i-4], font:this.profile.id, ...symbol
        });
      });
      const colons = clockFace ? [...svg.querySelectorAll<SVGPathElement>('path[data-c="3A"]')].map((path,i) =>
        glyphToken([path],{slot:`colon${i}`,kind:':',text:':',font:this.profile.id})) : [];
      const structures: PlacedToken[] = [];
      function ruleToken(rect: SVGRectElement, metadata: {site: string; kind: string; glyphKey: string}): PlacedToken {
      ...
      const coreOf = (tag: Element,name: string) => tag.matches(`[data-mml-node="${name}"]`) ? tag : tag.querySelector(`[data-mml-node="${name}"]`);
      const ownRules = (core: Element) => [...core.querySelectorAll('rect')].filter(rect => rect.closest('[data-mml-node="mfrac"],[data-mml-node="msqrt"]') === core);
      const singleGlyph = (path: SVGPathElement | undefined) => path?.hasAttribute('data-glyph-key') && path.closest('[data-mml-node="mo"]')?.querySelectorAll('path').length === 1;
      function takeGlyph(path: SVGPathElement, metadata: {site: string; kind: string; glyphKey: string}) {
      ...
      for (const tag of svg.querySelectorAll('[id^="fc-struct-"]')) {
        ...
        if (role === 'frac') {
          ...
          if (!core) continue;
          const paths = [...core.querySelectorAll<SVGPathElement>('path[data-c="221A"]')].filter(path => path.closest('[data-mml-node="msqrt"]') === core);
          ...
          const paths = [...tag.querySelectorAll<SVGPathElement>('path[data-c="28"],path[data-c="29"]')].filter(path => path.closest('[id^="fc-struct-"]') === tag);
