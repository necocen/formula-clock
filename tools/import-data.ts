...
if (!directory)
  throw new Error('Usage: node --import tsx tools/import-data.ts DIRECTORY_WITH_00_TO_23_JSON');
const lines: string[] = [];
let equations = 0;
for (let h = 0; h < 24; h++) {
  ...
  if (
    ...
    Object.keys(value).some((key) => !['schema', 'minutes'].includes(key)) ||
    Object.keys(value.minutes).length !== 60
  ) {
  ...
  for (let m = 0; m < 60; m++) {
    ...
    const minute = normalizeMinute(
      { schema: SCHEMA, hhmm: code, seconds: value.minutes[code] },
      code,
    );
