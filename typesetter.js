    ...
    load() {
      return new Promise((resolve, reject) => {
        ...
        script.onload = () => {
          ...
          startup.then(() => { this.mathjax = engineWindow.MathJax; return this.calibrateTypography(); }).then(() => finish(), finish);
        };
    ...
    async convert(tex, code, seconds) {
      ...
      try {
        ...
        const tokens = slots.map((slot, i) => {
          ...
          const paths = [...tagged.querySelectorAll('path')];
          ...
          for (const path of paths) {
            ...
            copy.removeAttribute('id'); copy.removeAttribute('transform');
