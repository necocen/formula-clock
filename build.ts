...
const bundle = (entry: string) => buildSync({absWorkingDir:root,entryPoints:[entry],bundle:true,
  write:false,format:'iife',platform:'browser',target:'es2022',legalComments:'inline'}).outputFiles[0].text;
...
html+=`\n<script id="browser-code">\n${bundle('browser.ts')}\n</script>\n`;
const data=read('data/expressions.json');
if(external) {
  const table: FormulaTable=JSON.parse(data);
  const folder=path.join(outDir,'data','hours');fs.mkdirSync(folder,{recursive:true});
  const hours: Record<string,string>={};
  const hash=(value: string)=>crypto.createHash('sha256').update(value).digest('hex');
  for(let h=0;h<24;h++) {
    const hour=String(h).padStart(2,'0'), minutes: Record<string,SecondEntries>={};
    for(let m=0;m<60;m++) {
...
if(external) execFileSync(process.execPath,['--import','tsx',path.join(root,'tools/build-worker.ts')],{cwd:root,stdio:'inherit'});
